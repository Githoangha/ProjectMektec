    ...
    public partial class frm_SettingReadCode : Form
    {
        ...
        private void btnNew_Click(object sender, EventArgs e)
        {
            ...
            if (dtRegionPosition.Rows.Count > 0)
            {
                ...
                {
                    number = Support_SQL.ToInt(dtRegionPosition.Rows[0]["Y1"]) - Support_SQL.ToInt(dtRegionPosition.Rows[1]["Y1"]);
                }
                else
                    number = 0;
                ...
                decimal Y1 = txtY1.Value = Support_SQL.ToDecimal(dtRegionPosition.Rows[0]["Y1"]) + number;
                decimal X2 = txtX2.Value = Support_SQL.ToDecimal(dtRegionPosition.Rows[0]["X2"]);
                decimal Y2 = txtY2.Value = Support_SQL.ToDecimal(dtRegionPosition.Rows[0]["Y2"]) + number;
        ...
        /// <summary>
        /// thêm mới vùng roi mới
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void btnNewSettingNoPCS_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                _drawing_object_Region_Setting_No_PCS = HDrawingObject.CreateDrawingObject(HDrawingObject.HDrawingObjectType.RECTANGLE1, 1504, 35, 1610, 164); //Color.Yellow
                _drawing_object_Region_Setting_No_PCS.SetDrawingObjectParams("color", "blue");//blue   
                _Window.AttachDrawingObjectToWindow(_drawing_object_Region_Setting_No_PCS);
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void btnGetGray_Click(object sender, EventArgs e)
        {
            GetRatioHaveOrNonePCS(txtRatioNonePCS);
        ...
        private void btnTrainAll_SettingNoPCS_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                for (int i = 0; i < countRegions; i++)
                {
                    ...
                    if (maxgrayValue.D > mingrayValue_Old.D)
                        mingrayValue_Old.D = maxgrayValue.D;
                }
                if (txtMaxGrayPCS.InvokeRequired)
                    txtMaxGrayPCS.Invoke(new Action(() => txtMaxGrayPCS.Text = Support_SQL.ToString(mingrayValue_Old.D)));
                else
                    txtMaxGrayPCS.Text = Support_SQL.ToString(mingrayValue_Old.D);
                GetRatioHaveOrNonePCS(txtRatioHavePCS);
                btnRatioNonePCS.Focus();
            }
            catch (Exception ex)
            {
        ...
        private void GetRatioHaveOrNonePCS(TextBox txt)
        {
            // TODO: NB - 31032023
            _Img?.Dispose();
            if (PLC_Fx5.Connected)
            ...
            for (int i = 0; i < countRegions; i++)
            {
                ...
                HOperatorSet.Threshold(ImgReduced, out HObject RegionP, 0, Convert.ToDouble(txtMaxGrayPCS.Text)); // nếu đèn sáng hơn thì tăng 50 lên. -> pixel đen
                HOperatorSet.ClosingRectangle1(RegionP, out RegionP, 9, 9);
                HOperatorSet.Connection(RegionP, out RegionP);
                HOperatorSet.SelectShapeStd(RegionP, out RegionP, "max_area", 60);
                ...
                HOperatorSet.Threshold(ImgReduced, out HObject RegionFull, 0, 255);
                HOperatorSet.AreaCenter(RegionFull, out HTuple AreaFull, out HTuple RowFull, out HTuple ColumnFull); // tổng roi
                double ratioItem = AreaP.D / AreaFull.D;
                string text = Support_SQL.ToString(Math.Round(ratioItem, 6) * 100) + "%";
                ...
                RatioPCS = RatioPCS + ratioItem;
            }
            double value = Math.Round(RatioPCS / countRegions, 6) * 100;
            if (txt.InvokeRequired)
            {
                txt.Invoke(new Action(() => txt.Text = Support_SQL.ToString(value)));
            }
            else
            {
                txt.Text = Support_SQL.ToString(value);
            }
            WindowControl.HalconWindow.ClearWindow();
            WindowControl.HalconWindow.DispObj(_Img);
        ...
        private void btnSaveSettingsNoPCS_Click(object sender, EventArgs e)
        {
            //check exist
            string sql = string.Empty;
            int ID = Support_SQL.ToInt(Support_SQL.ExecuteScalar($"SELECT ID FROM SettingNoPCS WHERE ID_Program = {ProgramID} AND CamIndex ={CamIndex}"));
            if (ID == 0)
            {
                sql = string.Format(@"insert into SettingNoPCS(ID_Program,CamIndex,MaxGrayPCS,RatioHavePCS,RatioNonePCS,RealRatioPCS) Values({0},{1},{2},{3},{4},{5})",
                    ProgramID, CamIndex, txtMaxGrayPCS.Text, txtRatioHavePCS.Text.Trim(), txtRatioNonePCS.Text.Trim(), txtRealRatioPCS.Text.Trim());
            }
            // sửa mới tọa độ
            else
            {
                sql = string.Format(@"update SettingNoPCS set MaxGrayPCS = {0}, RatioHavePCS = {1}, RatioNonePCS = {2}, RealRatioPCS = {3} where ID = {4}",
                    txtMaxGrayPCS.Text, txtRatioHavePCS.Text.Trim(), txtRatioNonePCS.Text.Trim(), txtRealRatioPCS.Text.Trim(), ID);
            }
            Support_SQL.ExecuteQuery(sql);
            if (_drawing_object_Region_Setting_No_PCS != null)
            {
            ...
            btnSave_SettingNoPCS.Enabled = false;
            btnNewSettingNoPCS.Focus();
        }
        /// <summary>
        ...
        {
            if (txtRatioHavePCS.Text == string.Empty)
            {
                new frm_ShowDialog(frm_ShowDialog.Icon_Show.Error, $"XIN VUI LÒNG KIỂM TRA TỈ LỆ JIG CÓ PCS[RatioHavePCS]", 3000).ShowDialog();
                return;
            }
            if (txtRatioNonePCS.Text == string.Empty)
            {
                new frm_ShowDialog(frm_ShowDialog.Icon_Show.Error, $"XIN VUI LÒNG KIỂM TRA TỈ LỆ JIG KHÔNG PCS[RatioNonePCS]", 3000).ShowDialog();
                return;
            }
            double value = (Convert.ToDouble(txtRatioHavePCS.Text) - Convert.ToDouble(txtRatioNonePCS.Text)) / 2;
            if (txtRealRatioPCS.InvokeRequired)
            {
                txtRealRatioPCS.Invoke(new Action(() => txtRealRatioPCS.Text = Support_SQL.ToString(value)));
            }
            else
            {
                txtRealRatioPCS.Text = Support_SQL.ToString(value);
            }
